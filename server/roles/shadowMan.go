...
func ShadowAttacks() map[AtKey]*AttackData {
	atks := make(map[AtKey]*AttackData)
	atks = shadowPrimaryAtk(atks)
	atks = shadowSecondaryAtk(atks)
...
func shadowPrimaryAtk(atks map[AtKey]*AttackData) map[AtKey]*AttackData {
...
func shadowSecondaryAtk(atks map[AtKey]*AttackData) map[AtKey]*AttackData {
	windup := &Windup{
		Duration: 200,
	}
